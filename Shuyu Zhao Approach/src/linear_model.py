...
from src import cameras as cam
...
class LinearModel(object):
    ...
    """
        ...
        with tf.variable_scope("inputs"):
            ...
            dec_out = tf.placeholder(
                dtype, shape=[None, self.output_size], name="dec_out"
            )
        ...
        with tf.variable_scope("linear_model"):
            ...
            b1 = tf.get_variable(
                name="b1", initializer=kaiming, shape=[linear_size], dtype=dtype
            )
            ...
            y3 = tf.nn.relu(y3)
            y3 = tf.nn.dropout(y3, 1 - (self.dropout_keep_prob))
            ...
            b4 = tf.get_variable(
                name="b4", initializer=kaiming, shape=[self.HUMAN_3D_SIZE], dtype=dtype
            )
        ...
        self.outputs = y
        self.loss = tf.reduce_mean(input_tensor=tf.square(y - dec_out))
        ...
        with tf.variable_scope("two_linear_" + str(idx)) as scope:
            ...
            if batch_norm:
                y = tf.layers.batch_normalization(
                    y, training=self.isTraining, name="batch_normalization1" + str(idx)
                )

            y = tf.nn.relu(y)
            y = tf.nn.dropout(y, 1 - (dropout_keep_prob))
            ...
            if batch_norm:
                y = tf.layers.batch_normalization(
                    y, training=self.isTraining, name="batch_normalization2" + str(idx)
                )
    ...
    """
        ...
        for key2d in data_x.keys():
            ...
            key3d = (
                (subj, b, fname[:-3])
                if fname.endswith("-sh") and camera_frame
                else key3d
            )
