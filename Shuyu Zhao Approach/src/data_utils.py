    ...
    for subj in subjects:
        for action in actions:
            ...
            dpath = os.path.join(
                bpath,
                "S{0}".format(subj),
                "MyPoses/{0}D_positions".format(dim),
                "{0}*.h5".format(action),
            )
            print(dpath)
            ...
            if dim == 2:
                assert (
                    loaded_seqs == 8
                ), "Expecting 8 sequences, found {0} instead".format(loaded_seqs)
            else:
                assert (
                    loaded_seqs == 2
                ), "Expecting 2 sequences, found {0} instead".format(loaded_seqs)
...
def load_stacked_hourglass(data_dir, subjects, actions):
  ...
  """
    ...
    assert np.all(
        SH_TO_GT_PERM
        == np.array([6, 2, 1, 0, 3, 4, 5, 7, 8, 9, 13, 14, 15, 12, 11, 10])
    )
    ...
    for subj in subjects:
        for action in actions:
            ...
            dpath = os.path.join(
                data_dir, "S{0}".format(subj), "StackedHourglass/{0}*.h5".format(action)
            )
            print(dpath)
                ...
                if seqname.startswith(action):
                    ...
                    with h5py.File(fname, "r") as h5f:
                        ...
                        dim_to_use_x = (
                            np.where(
                                np.array(
                                    [x != "" and x != "Neck/Nose" for x in H36M_NAMES]
                                )
                            )[0]
                            * 2
                        )
            ...
            if subj == 11 and action == "Directions":  # <-- this video is damaged
                assert (
                    loaded_seqs == 7
                ), "Expecting 7 sequences, found {0} instead. S:{1} {2}".format(
                    loaded_seqs, subj, action
                )
            else:
                assert (
                    loaded_seqs == 8
                ), "Expecting 8 sequences, found {0} instead. S:{1} {2}".format(
                    loaded_seqs, subj, action
                )
  ...
  """
    ...
    if dim == 2:
        dimensions_to_use = np.where(
            np.array([x != "" and x != "Neck/Nose" for x in H36M_NAMES])
        )[0]
...
def unNormalizeData(normalized_data, data_mean, data_std, dimensions_to_ignore):
  ...
  """
    ...
    dimensions_to_use = np.array(
        [dim for dim in range(D) if dim not in dimensions_to_ignore]
    )
