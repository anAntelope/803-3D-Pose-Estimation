...
from src import procrustes

from src import viz
from src import cameras
...
tf.flags.DEFINE_integer("batch_size", 64, "Batch size to use during training")
tf.flags.DEFINE_integer("epochs", 200, "How many epochs we should train for")
tf.flags.DEFINE_boolean("camera_frame", False, "Convert 3d poses to camera coordinates")
tf.flags.DEFINE_boolean("max_norm", False, "Apply maxnorm constraint to the weights")
tf.flags.DEFINE_boolean("batch_norm", False, "Use batch_normalization")

# Data loading
tf.flags.DEFINE_boolean("predict_14", False, "predict 14 joints")
...
# Architecture
tf.flags.DEFINE_integer("linear_size", 1024, "Size of each model layer.")
...
# Evaluation
tf.flags.DEFINE_boolean("procrustes", False, "Apply procrustes analysis at test time")
tf.flags.DEFINE_boolean(
...
# Directories
tf.flags.DEFINE_string(
    "cameras_path", "data/h36m/cameras.h5", "Directory to load camera parameters"
)
tf.flags.DEFINE_string("data_dir", "data/h36m/", "Data directory")
tf.flags.DEFINE_string("train_dir", "experiments", "Training directory.")

# Train or load
tf.flags.DEFINE_boolean("sample", False, "Set to True for sampling.")
tf.flags.DEFINE_boolean("use_cpu", False, "Whether to use the CPU")
tf.flags.DEFINE_integer("load", 0, "Try to load a previous checkpoint.")

# Misc
tf.flags.DEFINE_boolean("use_fp16", False, "Train using fp16 instead of fp32.")
...
def create_model(session, actions, batch_size):
  ...
  """
    ...
    print("train_dir mtfk", train_dir)

    if ckpt and ckpt.model_checkpoint_path:
        # Check if the specific checkpoint exists
        if FLAGS.load > 0:
            if os.path.isfile(
                os.path.join(train_dir, "checkpoint-{0}.index".format(FLAGS.load))
            ):
                ckpt_name = os.path.join(
                    os.path.join(train_dir, "checkpoint-{0}".format(FLAGS.load))
                )
            else:
                raise ValueError(
...
def train():
    ...
    if FLAGS.use_sh:
        (
            ...
            dim_to_ignore_2d,
            dim_to_use_2d,
        ) = data_utils.read_2d_predictions(actions, FLAGS.data_dir)
    else:
        (
            train_set_2d,
            test_set_2d,
    ...
    with tf.Session(
        config=tf.ConfigProto(device_count=device_count, allow_soft_placement=True)
    ) as sess:
        ...
        for _ in xrange(FLAGS.epochs):
            ...
            for i in range(nbatches):
                ...
                if (i + 1) % log_every_n_batches == 0:
                    ...
                    print(
                        "Working on epoch {0}, batch {1} / {2}... ".format(
            ...
            if FLAGS.evaluateActionWise:
                ...
                print(
                    "{0:=^12} {1:=^6}".format("Action", "mm")
                )  # line of 30 equal signs
                ...
                print(
                    "{0:<12} {1:>6.2f}".format("Average", cum_err / float(len(actions)))
                )
                print("{0:=^19}".format(""))
                ...
                for i in range(n_joints):
                    ...
                    print(
                        "Error in joint {0:02d} (mm): {1:>5.2f}".format(
                            i + 1, joint_err[i]
                        )
                    )
                print("=============================")
            ...
            start_time = time.time()
            model.saver.save(
                sess, os.path.join(train_dir, "checkpoint"), global_step=current_step
            )
  ...
  """
    ...
    for i in range(nbatches):
        ...
        if current_epoch > 0 and (i + 1) % log_every_n_batches == 0:
            print(
                "Working on test epoch {0}, batch {1} / {2}".format(
                    current_epoch, i + 1, nbatches
                )
            )
        ...
        dtu3d = (
            np.hstack((np.arange(3), dim_to_use_3d))
            if not (FLAGS.predict_14)
            else dim_to_use_3d
        )
        ...
        if FLAGS.procrustes:
            ...
            for j in range(FLAGS.batch_size):
                ...
                poses3d[j, :] = (
                    np.reshape(out, [-1, 17 * 3])
                    if not (FLAGS.predict_14)
                    else np.reshape(out, [-1, 14 * 3])
                )
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count)) as sess:
        ...
        for key2d in test_set_2d.keys():
            ...
            key3d = (
                ...
                else (subj, b, "{0}.h5".format(fname.split(".")[0]))
            )
            key3d = (
                (subj, b, fname[:-3])
                if (fname.endswith("-sh")) and FLAGS.camera_frame
                else key3d
            )
            ...
            if FLAGS.camera_frame:
                ...
                scams = {
                    (subj, c + 1): rcams[(subj, c + 1)] for c in range(N_CAMERAS)
                }  # cams of this subject
                scam_idx = [scams[(subj, c + 1)][-1] for c in range(N_CAMERAS)].index(
                    cname
